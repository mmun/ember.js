...
export function readViewFactory(object, container) {
  ...
  if (typeof value === 'string') {
    if (isGlobal(value)) {
      ...
      deprecate('Resolved the view "'+value+'" on the global context. Pass a view name to be looked up on the container instead, such as {{view "select"}}.', !viewClass, { url: 'http://emberjs.com/guides/deprecations/#toc_global-lookup-of-views' });
    } else {
  ...
  assert(fmt(value+' must be a subclass or an instance of Ember.View, not %@', [viewClass]), (function(viewClass) {
...
export function readComponentFactory(nameOrStream, container) {
  ...
  assert('Could not find \'component-lookup:main\' on the provided container,' +
